...
public class FetchTasksTest extends ServerTestCase {
  ...
  public static class TestFetchTaskPlugin extends Plugin implements FetchTaskPlugin {
    ...
    public Map<String, FetchTaskProvider<? extends FetchTasks.FetchTask>> getFetchTasks() {
      ...
      taskMap.put("segment_hits_task", new TestSegmentHitsProvider());
    ...
    public static class TestSegmentHitsProvider implements FetchTaskProvider<TestSegmentHitsTask> {
      @Override
      public TestSegmentHitsTask get(Map<String, Object> params) {
        return new TestSegmentHitsTask();
      }
    }

    public static class TestSegmentHitsTask implements FetchTasks.FetchTask {
      @Override
      public void processSegmentHits(
          ...
          throws IOException {
        FieldDef fd = searchContext.queryFields().get("int_field");
        LoadedDocValues<?> docValues = ((IndexableFieldDef) fd).getDocValues(segmentContext);
        for (SearchResponse.Hit.Builder hit : hits) {
          ...
          hit.putFields(
              ...
              SearchResponse.Hit.CompositeFieldValue.newBuilder()
                  .addFieldValue(
                      SearchResponse.Hit.FieldValue.newBuilder().setIntValue(-fieldVal).build())
                  .build());
        }
      }
    }
  ...
  @Test
  public void testFetchTaskSegmentHits() {
    FetchTask grpcFetchTask = FetchTask.newBuilder().setName("segment_hits_task").build();
    SearchResponse response = doQuery(Collections.singletonList(grpcFetchTask));
    assertBaseHit(response);
    for (int i = 0; i < response.getHitsCount(); ++i) {
      SearchResponse.Hit hit = response.getHits(i);
      ...
      assertEquals(3, hit.getFieldsCount());
      assertEquals(-fieldVal, hit.getFieldsOrThrow("segment_task").getFieldValue(0).getIntValue());
    }
  }
  ...
  public void testMultipleFetchTasks() {
    ...
    FetchTask grpcFetchTask2 = FetchTask.newBuilder().setName("segment_hits_task").build();
    FetchTask grpcFetchTask3 = FetchTask.newBuilder().setName("hits_task").build();
    SearchResponse response =
        doQuery(Arrays.asList(grpcFetchTask1, grpcFetchTask2, grpcFetchTask3));
    assertBaseHit(response);
    for (int i = 0; i < response.getHitsCount(); ++i) {
      SearchResponse.Hit hit = response.getHits(i);
      int fieldVal = hit.getFieldsOrThrow("int_field").getFieldValue(0).getIntValue();
      assertEquals(5, hit.getFieldsCount());
      assertEquals(
          hit.getLuceneDocId() - 10,
          hit.getFieldsOrThrow("all_task").getFieldValue(0).getIntValue());
      assertEquals(-fieldVal, hit.getFieldsOrThrow("segment_task").getFieldValue(0).getIntValue());
