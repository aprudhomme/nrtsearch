...
class SegmentInfosSearcherManager extends ReferenceManager<IndexSearcher> {
  private static final Logger logger = LoggerFactory.getLogger(SegmentInfosSearcherManager.class);
  ...
  public SegmentInfosSearcherManager(
    ...
    current =
        SearcherManager.getSearcher(
            searcherFactory,
            StandardDirectoryReader.open(dir, currentInfos, null, Collections.emptyMap()),
            null);
  ...
  protected IndexSearcher refreshIfNeeded(IndexSearcher old) throws IOException {
    final SegmentInfos newInfos = currentInfos;
    ...
    if (old == null) {
      ...
      logger.info("Refreshing search, old segments: " + old.getIndexReader().leaves());
      subs = new ArrayList<>();
