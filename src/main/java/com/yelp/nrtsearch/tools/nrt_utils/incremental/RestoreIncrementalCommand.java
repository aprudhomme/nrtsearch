...
public class RestoreIncrementalCommand implements Callable<Integer> {
  ...
  private void restoreIndexState(
    ...
    // the state archive must be rewritten to have the internal directory name match
    // the restored index name
    String stateContents =
        StateCommandUtils.getStateFileContentsFromS3(
            s3Client,
    ...
    String rewrittenState = rewriteStateWithRestoreIndexName(stateContents);
    byte[] archiveData =
        StateCommandUtils.buildStateFileArchive(
            restoreIndexResource, StateUtils.INDEX_STATE_FILE, StateUtils.toUTF8(rewrittenState));
    StateCommandUtils.writeDataToS3(
        s3Client, versionManager.getBucketName(), restoreStateKey, archiveData);
    versionManager.blessVersion(restoreServiceName, restoreIndexStateResource, stateFileId);
  }

  private String rewriteStateWithRestoreIndexName(String contents) throws IOException {
    IndexStateInfo.Builder stateBuilder = IndexStateInfo.newBuilder();
    JsonFormat.parser().merge(contents, stateBuilder);
    stateBuilder.setIndexName(restoreIndexName);
    return JsonFormat.printer().print(stateBuilder.build());
  }
