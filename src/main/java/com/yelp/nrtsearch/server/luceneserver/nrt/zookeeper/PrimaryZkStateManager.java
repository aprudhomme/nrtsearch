...
public class PrimaryZkStateManager extends ZkStateManager implements PrimaryStateManager {
  ...
  public PrimaryZkStateManager(String indexName, int shardOrd, String serviceName, long primaryGen)
      throws IOException {
    ...
    localMergeState =
        new MergeState(ephemeralId.toString(), primaryGen, new HashMap<>(), new HashMap<>());
    initMergeState();
  ...
  private final StatCallback updateActiveStateCallback =
      (rc, path, ctx, stat) -> {
        if (Code.get(rc) == Code.CONNECTIONLOSS) {
          updateActiveState((ActiveState) ctx);
        }
      };

  private synchronized void updateActiveState(ActiveState newActiveState) {
    if (activeState == newActiveState) {
      ...
      getZk()
          .setData(
              getActiveStatePath(),
              activeStateJson.getBytes(),
              -1,
              updateActiveStateCallback,
              newActiveState);
    } else {
  ...
  private final StatCallback updateMergeStateCallback =
      (rc, path, ctx, stat) -> {
        if (Code.get(rc) == Code.CONNECTIONLOSS) {
  ...
  private final Watcher replicasChangedWatcher =
      event -> {
  ...
  private final ChildrenCallback getReplicasCallback =
      (rc, path, ctx, children) -> {
        switch (Code.get(rc)) {
          ...
          default:
            System.out.println(
                "Failed to get replicas: " + KeeperException.create(Code.get(rc), path));
        }
      };
  ...
  private final Watcher replicaStateWatcher =
      event -> {
  ...
  private final DataCallback replicaStateCallback =
      (rc, path, ctx, data, stat) -> {
        ...
        switch (Code.get(rc)) {
          ...
          default:
            System.out.println(
                "Failed to get replica state: " + KeeperException.create(Code.get(rc), path));
        }
      };
  ...
  private synchronized void addMergeReplica(String replica) {
  ...
  private synchronized void removeMergeReplica(String replica) {
    MergeState newMergeState = new MergeState(localMergeState);
