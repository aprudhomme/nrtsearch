 ...
 * @lucene.experimental
 */
public class MySearcherLifetimeManager implements Closeable {
  private static final Logger logger = LoggerFactory.getLogger(MySearcherLifetimeManager.class);
  ...
  private static class SearcherTracker implements Comparable<SearcherTracker>, Closeable {
    ...
    public SearcherTracker(SearcherTracker other) {
  ...
  private final Set<SearcherTracker> searcherCollisions =
  ...
  public long record(IndexSearcher searcher) throws IOException {
    ...
    if (tracker == null) {
      ...
      tracker = new SearcherTracker(searcher);
      SearcherTracker previous = searchers.put(version, tracker);
      if (previous != null) {
        if (previous.searcher == searcher) {
          tracker.close();
        } else {
          logger.info(
              "Replaced previous searcher for version: "
                  + version
                  + " (new="
                  + searcher
                  + " vs old="
                  + previous.searcher
                  + ")");
          searcherCollisions.add(new SearcherTracker(previous));
        }
      }
    }
  ...
  /**
   * Calls provided {@link MySearcherLifetimeManager.Pruner} to prune entries. The entries are
   * passed to the Pruner in sorted (newest to oldest IndexSearcher) order.
   *
   * <p><b>NOTE</b>: you must periodically call this, ideally from the same background thread that
   * opens new searchers.
   */
  public synchronized void prune(MySearcherLifetimeManager.Pruner pruner) throws IOException {
    ...
    if (!searcherCollisions.isEmpty()) {
      List<SearcherTracker> collisionTrackers = new ArrayList<>(searcherCollisions);
      for (SearcherTracker tracker : collisionTrackers) {
        double ageSec = now - tracker.recordTimeSec;
        if (pruner.doPrune(ageSec, tracker.searcher)) {
          searcherCollisions.remove(tracker);
          tracker.close();
        }
      }
    }
